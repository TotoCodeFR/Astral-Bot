...
const logs: Record<string, { getEmbed: (interaction: any, fields: Record<string, string>) => EmbedBuilder }> = {
    ban: {
...
const interactiveDM: Record<string, InteractiveDmSection> = {
    home: {
        name: "Accueil",
        emoji: "🏠",
...
for (const [key, value] of Object.entries(interactiveDM)) {
    if (!value?.name || key === "loading") continue;
...
const homeRow = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
    createSelectMenu("home"),
);
const serverGuideRow =
    new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
        createSelectMenu("serverGuide"),
    );
const contactStaffRow = [
    new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
        createSelectMenu("contactStaff"),
    ),
